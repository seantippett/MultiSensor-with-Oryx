...
extern unsigned int __data_section_table_BL;
extern unsigned int __data_section_table_end_BL;
extern unsigned int __bss_section_table_BL;
extern unsigned int __bss_section_table_end_BL;

extern void checkBootload(void);
void ResetISR(void);
...
void ResetISR_BL(void){
    ...
    SectionTableAddr = &__data_section_table_BL;
    // Copy the data sections from flash to SRAM.
    while (SectionTableAddr < &__data_section_table_end_BL) {
        LoadAddr = *SectionTableAddr++;
        ExeAddr = *SectionTableAddr++;
        SectionLen = *SectionTableAddr++;
        data_init(LoadAddr, ExeAddr, SectionLen);
    }
    while (SectionTableAddr < &__bss_section_table_end_BL) {
        ExeAddr = *SectionTableAddr++;
        SectionLen = *SectionTableAddr++;
    ...
    //
    // ResetISR() shouldn't return, but if it does, we'll just enter an infinite loop
    //
    while (1) {
        ;
    }
...
//__attribute__ ((naked, section(".after_vectors.reset")))			<-  This is what the line used to be.  But this places the ResetISR in the BL section (we don't want this).

__attribute__ ((naked, section(".text_BOARD_FLASH")))				// <-  This is what we want.  It forces ResetISR to the normal flash section.  This is because our main() location... our variable defines... those can all change.
//__TEXT(BOARD_FLASH)
void ResetISR(void) {
    // Disable interrupts
    __asm volatile ("cpsid i");
